...
Now we want to write another function: `add`. This function will take two numbers and return their sum. For the sake of the exercise, it is not allowed to use the `+` operator -- only the `increment` function we've already written and loops and if statements. First, let's write a test for the function:
...
- Create a variable `solution` and make it `x` to start
...
Uh oh! This test failed. It says the actual value we got was `4` but the expected value was `1`. Let's fix this. We'll fix it by creating a `decrement` functiont hat looks like the `increment` function:
...
describe('index.js', () => {
    ...
    describe('decrementEach()', () => {
        it('decreases each element', () => {
            expect(decrementEach(1, 2, 3)).to.eql([1, 2, 3]);
        });
    });
});
```
